...
Param(
  ...
  # DKIM from Resend:
  # prefix = фиксированная часть DKIM
  # key    = значение из Resend ("p=MIGfMA0GCSqGSIb3...")
  [string]$ResendDkimPrefix = "v=DKIM1; k=rsa;"
  [string]$ResendDkimKey    = "p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQD..."   # вставь своё
...
function Reset-DnsTransaction {
    param([string]$ZoneName)

    # Попробуем аккуратно абортнуть любую незавершённую транзакцию
    try {
        Invoke-Gcloud "dns record-sets transaction abort --zone=$ZoneName" -SuccessCode 0 | Out-Null
    } catch {
        # Если транзакции нет – тоже ок, молча игнорируем
    }

    # И на всякий случай удалим локальный transaction.yaml в текущей папке
    try {
...
if ($currentDkim) {
  ...
  if (-not $WhatIf) {
    Reset-DnsTransaction -ZoneName $DnsZoneName
    Invoke-Gcloud "dns record-sets transaction start   --zone=$DnsZoneName" | Out-Null
